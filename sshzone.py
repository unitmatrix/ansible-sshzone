# Copyright (c) 2019-2020, Aleksandr Cupacenko (@unitmatrix)
...
DOCUMENTATION = '''
    connection: sshzone
    short_description: connect via ssh client binary to zone
    description:
        - This connection plugin allows ansible to communicate to the target machines via normal ssh command line.
    author: Aleksandr Cupacenko (@unitmatrix)
    ...
    options:
      ...
      zone_name:
        ...
        vars:
          - name: ansible_zone_name
      zone_zoneadm_exe:
        default: /usr/sbin/zoneadm
        description:
          - full path to zoneadm binary
        vars:
          - name: ansible_zone_zoneadm_exe
      zone_zlogin_exe:
        default: /usr/sbin/zlogin
        description:
          - full path to zlogin binary
        vars:
          - name: ansible_zone_zlogin_exe
'''
...
class Connection(ConnectionBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # zone information loaded on first use by match_zone
        self.zid = None
        self.zname = None
        self.zpath = None
        self.zstatus = None
    ...
    def match_zone(self):
        if self.zid is None:
            code, stdout, stderr = self._zonehost_command("%s list -civ" % (self.get_zoneadm()))
            if code != 0:
                display.vvv("zoneadm stdout: %s" % stdout)
                raise AnsibleError("zoneadm returned non-zero!")
            ...
            #  1 yzerman          running     /system/zones/yzerman        solaris10  excl
            #  2 messier          running     /system/zones/messier        solaris10  excl
            #  7 kariya           running     /system/zones/kariya         solaris10  excl

            for line in lines[1:]: # skip header line
                id, name, status, path, brand, ip = to_text(line).strip().split()
                if name == self.get_option('zone_name'):
                    self.zid = id
                    self.zname = name
                    self.zpath = os.path.join(path, 'root')
                    self.zstatus = status
                    found = True
                    break

            if not found:
                raise AnsibleError("failed to find a zone with name of '%s'" % self.get_option('zone_name'))
                
            if self.zstatus != 'running':
                raise AnsibleError("zone '%s' status is '%s', status must be 'running' to log in." % (self.zname, self.zstatus))
    ...
    def get_zoneadm(self):
        if self.zoneadm is None:
            self.zoneadm = self.get_option('zone_zoneadm_exe');
        return self.zoneadm

    def get_zlogin(self):
        if self.zlogin is None:
            self.zlogin = self.get_option('zone_zlogin_exe')
        return self.zlogin
    ...
    def _zonehost_command(self, cmd):
    ...
    def exec_command(self, cmd, in_data=None, executable='/bin/sh', sudoable=True):
        ...
        cmd = pipes.quote(cmd)
        if slpcmd:
            cmd = '%s %s %s %s' % (self.get_zlogin(), self.get_zone_name(), cmd, '&& sleep 0')
        else:
    ...
    def _copy_file(self, from_file, to_file, executable='/bin/sh'):
        ...
        display.vvv(u"REMOTE COPY {0} TO {1}".format(from_file, to_file), host=self.host)
        code, stdout, stderr = self._zonehost_command(copycmd)
        if code != 0:
            raise AnsibleError("failed to copy file from %s to %s:\n%s\n%s" % (from_file, to_file, stdout, stderr))
    
    @contextmanager
    def tempfile(self):
        code, stdout, stderr = self._zonehost_command('mktemp')
        if code != 0:
            raise AnsibleError("failed to make temp file:\n%s\n%s" % (stdout, stderr))
        tmp = to_text(stdout.strip().split(b'\n')[-1])
    
        code, stdout, stderr = self._zonehost_command(' '.join(['chmod 0644', tmp]))
        if code != 0:
            raise AnsibleError("failed to make temp file %s world readable:\n%s\n%s" % (tmp, stdout, stderr))
    
        yield tmp
    
        code, stdout, stderr = self._zonehost_command(' '.join(['rm', tmp]))
        if code != 0:
            raise AnsibleError("failed to remove temp file %s:\n%s\n%s" % (tmp, stdout, stderr))

    def put_file(self, in_path, out_path):
        ''' transfer a file from local to remote zone '''
